...
from torch.distributions import Poisson, Gamma, NegativeBinomial
...
class HurdleShiftedPoisson(ExponentialFamily):
    ...
    def __init__(self, rate, pi, validate_args=None):
        self.rate, self.pi = broadcast_all(rate, pi)
        if isinstance(rate, Number) and isinstance(pi, Number):
            batch_shape = torch.Size()
        else:
    ...
    def log_prob(self, value):
        value = torch.as_tensor(value, dtype=self.rate.dtype, device=self.pi.device)
        if self._validate_args:
            self._validate_sample(value)
            
        value, rate, pi = broadcast_all(value, self.rate, self.pi)
        ...
        if torch.any(zeros):
            log_p[zeros] = torch.log(pi[zeros])
        
        if torch.any(non_zeros):
            log_p[non_zeros] = torch.log(1-pi[non_zeros]) + Poisson(rate[non_zeros]).log_prob(value[non_zeros] - 1)
        
        return log_p
    
    def rsample(self, sample_shape=torch.Size()):
        
        rate, pi = broadcast_all(self.rate, self.pi)

        with torch.no_grad():
            
            return torch.bernoulli(torch.broadcast_to(1-pi, sample_shape + pi.shape))*(1+Poisson(rate).sample(sample_shape))
...
class HurdleShiftedNegativeBinomial(ExponentialFamily):
    ...
    def __init__(self, total_count, probs, pi, validate_args=None):
        self.total_count, self.probs, self.pi = broadcast_all(total_count, probs, pi)
        if isinstance(total_count, Number) and isinstance(probs, Number) and isinstance(pi, Number):
            batch_shape = torch.Size()
        else:
    ...
    def log_prob(self, value):
        value = torch.as_tensor(value, dtype=self.total_count.dtype, device=self.pi.device)
        if self._validate_args:
            self._validate_sample(value)
            
        value, total_count, probs, pi = broadcast_all(value, self.total_count, self.probs, self.pi)
        ...
        if torch.any(non_zeros):
            log_p[non_zeros] = torch.log(1-pi[non_zeros]) + NegativeBinomial(total_count[non_zeros], probs[non_zeros]).log_prob(value[non_zeros] - 1)
        
        return log_p
    
    def rsample(self, sample_shape=torch.Size()):
        
        total_count, probs, pi = broadcast_all(self.total_count, self.probs, self.pi)

        with torch.no_grad():
            
            return torch.bernoulli(torch.broadcast_to(1-pi, sample_shape + pi.shape))*(1+NegativeBinomial(total_count, probs).sample(sample_shape))
...
class ZeroInflatedPoisson(ExponentialFamily):
    ...
    def log_prob(self, value):
        ...
        return torch.where(value == 0, torch.log(pi + (1-pi)*distr.log_prob(torch.tensor([0.])).exp()), torch.log(1-pi)+distr.log_prob(value))
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        with torch.no_grad():
            ...
            return torch.bernoulli(torch.broadcast_to(1-pi, sample_shape + pi.shape))*(Poisson(rate).sample(sample_shape))
...
class ZeroInflatedNegativeBinomial(ExponentialFamily):
    ...
    def log_prob(self, value):
        ...
        distr = NegativeBinomial(total_count, probs)
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        with torch.no_grad():
            ...
            return torch.bernoulli(torch.broadcast_to(1-pi, sample_shape + pi.shape))*(NegativeBinomial(total_count, probs).sample(sample_shape))
...
class ZeroInflatedGamma(ExponentialFamily):
    ...
    def __init__(self, concentration, rate, pi, validate_args=None):
        self.concentration, self.rate, self.pi = broadcast_all(concentration, rate, pi)
        if isinstance(concentration, Number) and isinstance(rate, Number) and isinstance(pi, Number):
            batch_shape = torch.Size()
        else:
    ...
    def log_prob(self, value):
        value = torch.as_tensor(value, dtype=self.concentration.dtype, device=self.pi.device)
        if self._validate_args:
            self._validate_sample(value)
            
        value, concentration, rate, pi = broadcast_all(value, self.concentration, self.rate, self.pi)
        
        distr = Gamma(concentration, rate)
        
        return torch.where(value == 0, torch.log(pi), torch.log(1-pi)+distr.log_prob(value))
    
    def rsample(self, sample_shape=torch.Size()):
        
        concentration, rate, pi = broadcast_all(self.concentration, self.rate, self.pi)

        with torch.no_grad():
            
            return torch.bernoulli(torch.broadcast_to(1-pi, sample_shape + pi.shape))*(Gamma(concentration, rate).sample(sample_shape))
