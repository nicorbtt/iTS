...
from gluonts.torch.scaler import Scaler, MeanScaler, NOPScaler, MeanDemandScaler, MASEScaler
...
class DeepARModel(nn.Module):
    ...
    *Note:* the code of this model is unrelated to the implementation behind
    ...
    lags_seq
        ...
        and ``t-25`` as input.
    scaling
        The kind of scaling to be applied.
    default_scale
    ...
    def __init__(
        ...
        if scaling == True or scaling == 'mean':
            self.scaler: Scaler = MeanScaler(
                dim=-1, keepdim=True, default_scale=default_scale, minimum_scale=1.,
            )
        elif scaling == 'mean demand':
            self.scaler: Scaler = MeanDemandScaler(
                dim=-1, keepdim=True, default_scale=default_scale, minimum_scale=1., 
            )
        elif scaling == 'MASE':
            self.scaler: Scaler = MeanScaler(
