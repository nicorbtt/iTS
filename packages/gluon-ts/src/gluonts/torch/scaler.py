...
class MeanDemandScaler(Scaler):
    """
    Computes a scaling dividing by the mean of non-zero values, along dimension ``dim``, and scales the data accordingly.
    ...
    def __call__(
        ...
        # shape: (N, [C], T=1)
        ts_sum = (data * observed_indicator).sum(self.dim, keepdim=True)
