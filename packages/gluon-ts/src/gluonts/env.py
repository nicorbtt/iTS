...
class Environment(Settings):
    ...
    max_idle_transforms: int = os.environ.get(  # type: ignore
        "GLUONTS_MAX_IDLE_TRANSFORMS", "10000"
    )

    sample_zero_percentage: float = os.environ.get(  # type: ignore
        "iTS_sample_zero_percentage", "1"
    )

    p_sample_zero_percentage_reject: float = os.environ.get(  # type: ignore
        "iTS_p_sample_zero_percentage_reject", "1"
    )
